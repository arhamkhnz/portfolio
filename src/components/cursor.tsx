...
export function Cursor() {
  ...
  useEffect(() => {
    ...
    const onMouseMove = (event: MouseEvent) => {
      animate(
        bigBall,
        { x: event.pageX - 15, y: event.pageY - 15 },
        { type: "tween", duration: 0.4, ease: power1Out },
      );
      animate(
        smallBall,
        { x: event.pageX - 5, y: event.pageY - 7 },
        { type: "tween", duration: 0.1, ease: power1Out },
      );
    };
    ...
    const onMouseHover = () => {
      animate(
        bigBall,
        { scale: 4 },
        { type: "tween", duration: 0.3, ease: power1Out },
      );
    };
  ...
  return (
    <div ref={scope} className="pointer-events-none" aria-hidden="true">
      <div
        ref={bigBallRef}
        className="fixed top-0 left-0 z-[9999] mix-blend-difference [.hide-cursor_&]:hidden"
      >
